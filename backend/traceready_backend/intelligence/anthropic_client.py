...
class AnthropicLLMConfig:
    ...
    def from_env(cls) -> "AnthropicLLMConfig":
        ...
        return cls(
            ...
            conflict_model=os.getenv("TRACEREADY_ANTHROPIC_CONFLICT_MODEL", DEFAULT_ANTHROPIC_CONFLICT_MODEL).strip()
            or DEFAULT_ANTHROPIC_CONFLICT_MODEL,
            max_tokens=int(os.getenv("TRACEREADY_ANTHROPIC_MAX_TOKENS", str(DEFAULT_MAX_TOKENS))),
            temperature=float(os.getenv("TRACEREADY_ANTHROPIC_TEMPERATURE", "0")),
            prompt_cache_enabled=os.getenv("TRACEREADY_ANTHROPIC_PROMPT_CACHE", "1").strip().lower()
            not in {"0", "false", "no", "off"},
            prompt_cache_ttl=_normalized_cache_ttl(_env_optional("TRACEREADY_ANTHROPIC_PROMPT_CACHE_TTL")),
        )
    ...
    def __init__(self, config: AnthropicLLMConfig):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "or `python -m pip install anthropic` inside traceready/ingestion."
            ) from exc
...
def _normalized_cache_ttl(value: str | None) -> str | None:
    if value is None:
        return None
    if value not in {"1h"}:
        raise ValueError("TRACEREADY_ANTHROPIC_PROMPT_CACHE_TTL currently supports only blank/default or '1h'.")
    return value
