...
FIELD_ALIASES: dict[str, tuple[str, str]] = {
    ...
    # Trakkey / Sea Eagle real-export column variants (learned from issue #2 validation).
    "product title": ("product_name", "Product name"),
    "producttitle": ("product_name", "Product name"),
    "product_title": ("product_name", "Product name"),
    "ftl group": ("ftl_category", "Food Traceability List category"),
    "ftl_group": ("ftl_category", "Food Traceability List category"),
    "lot assigned": ("traceability_lot_code", "Traceability lot code"),
    "lot_assigned": ("traceability_lot_code", "Traceability lot code"),
    "source location id": ("from_partner_id", "Source location (supplier) identifier"),
    "source_location_id": ("from_partner_id", "Source location (supplier) identifier"),
    "destination location id": ("to_partner_id", "Destination location identifier"),
    "destination_location_id": ("to_partner_id", "Destination location identifier"),
    "harvesting date": ("event_datetime", "Traceability event date/time"),
    "cooling date": ("event_datetime", "Traceability event date/time"),
    "landing date": ("event_datetime", "Traceability event date/time"),
    "packing date": ("event_datetime", "Traceability event date/time"),
    "receiving date": ("received_date", "Receiving event date"),
    "transformation date": ("event_datetime", "Traceability event date/time"),
...
def _mapping_rationale(column_name: str, field_key: str, confidence: float) -> str:
    if confidence >= 0.9:
        return f"Column {column_name!r} matched a known Bellwether/FSMA field alias for {field_key!r}."
