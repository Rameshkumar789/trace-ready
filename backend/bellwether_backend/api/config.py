...
class ServiceSettings(BaseModel):
    service_name: str = "bellwether-python-backend"
    service_version: str = "0.1.0"
    environment: RuntimeEnvironment = RuntimeEnvironment.LOCAL
    database_url: str | None = None
    supabase_url: str | None = None
    supabase_service_role_key: str | None = None
    supabase_storage_bucket: str = "bellwether-pilot-private"
    object_store_mode: ObjectStoreMode | None = None
    local_object_store_root: str = ".bellwether-object-store"
...
def _read_environment(environ: Mapping[str, str]) -> RuntimeEnvironment:
    explicit = environ.get("BELLWETHER_ENV")
...
def load_settings(environ: Mapping[str, str] | None = None) -> ServiceSettings:
    ...
    return ServiceSettings(
        ...
        supabase_storage_bucket=env.get("BELLWETHER_STORAGE_BUCKET", "bellwether-pilot-private"),
        object_store_mode=_read_object_store_mode(env),
        local_object_store_root=env.get("BELLWETHER_LOCAL_OBJECT_STORE_ROOT", ".bellwether-object-store"),
        internal_api_token=env.get("BELLWETHER_INTERNAL_API_TOKEN"),
        require_configured_dependencies=_read_bool(
            env.get("BELLWETHER_REQUIRE_CONFIGURED_DEPENDENCIES")
        ),
        allowed_origins=_read_csv(env.get("BELLWETHER_ALLOWED_ORIGINS")),
    )
...
def _read_object_store_mode(environ: Mapping[str, str]) -> ObjectStoreMode | None:
    raw = environ.get("BELLWETHER_OBJECT_STORE_MODE")
    if not raw:
        return None
    try:
