...
export default async function AuditWorkspacePage({ params }: { params: Promise<{ auditId: string }> }) {
  ...
  if (!session || !canAccessPath(session, `/operator/audits/${auditId}`)) {
    redirect(`/login/operator?auth=required&next=${encodeURIComponent(`/operator/audits/${auditId}`)}`);
  }
  ...
  if (auditId !== "demo") {
    ...
    if (!ready) {
      return (
        <BellwetherShell active="audits" topbarLeft="AUDIT · PROCESSING">
  ...
  return (
    <BellwetherShell active="audits" topbarLeft={`AUDIT · ${audit.fileName}`}>
      <div className="flex flex-col gap-6" style={{ padding: 28 }}>
        <nav className="flex items-center gap-1.5 text-[13px] text-muted" aria-label="Breadcrumb">
          <Link href="/operator/audits" className="inline-flex items-center gap-1 text-muted no-underline hover:text-ink">
        ...
        {toReview.length ? (
          <section className="flex flex-col gap-3">
            <SectionHeader
              ...
              hint="Bellwether won't guess a pass/fail"
            />
