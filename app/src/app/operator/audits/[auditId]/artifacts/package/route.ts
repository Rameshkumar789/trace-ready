...
export async function GET(_request: Request, { params }: { params: Promise<{ auditId: string }> }) {
  ...
  if (!session || !canAccessPath(session, `/operator/audits/${auditId}/artifacts/package`)) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  if (auditId === "demo") {
    return jsonDownload(buildAuditExportPackage(demoAsStored()), `bellwether-audit-package-${auditId}.json`);
  }
