...
export async function uploadWorkbookAction(formData: FormData) {
  ...
  if (!session || !canAccessPath(session, "/operator/upload")) {
    redirect("/login/operator?auth=required&next=/operator/upload");
  }

  const file = formData.get("workbook");
  if (!(file instanceof File)) {
    redirect("/operator/upload?error=missing_file");
  }

  const uploadValidation = validateUploadMetadata(file.name, file.size);
  if (!uploadValidation.valid) {
    redirect(`/operator/upload?error=${encodeURIComponent(uploadValidation.errors.join("; "))}`);
  }
  ...
  try {
    ...
    redirect(`/operator/upload?error=${encodeURIComponent("Upload could not be queued. Check Supabase configuration and try again.")}`);
  }
  ...
  redirect(`/operator/audits/${queued.auditProjectId}`);
}

async function requestPythonWorkerSlice(userId: string) {
  const pythonApiUrl = (process.env.BELLWETHER_PYTHON_API_URL ?? "http://127.0.0.1:8000").replace(/\/+$/u, "");
  const internalToken = process.env.BELLWETHER_INTERNAL_API_TOKEN;
  if (!internalToken) {
    throw new Error("BELLWETHER_INTERNAL_API_TOKEN is missing in the Next.js app environment.");
  }
  const response = await fetch(`${pythonApiUrl}/internal/jobs/audit/process-slice`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "x-bellwether-internal-token": internalToken
    },
    body: JSON.stringify({
