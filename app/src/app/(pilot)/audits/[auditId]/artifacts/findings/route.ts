...
export async function GET(_request: Request, { params }: { params: Promise<{ auditId: string }> }) {
  ...
  if (!session || !canAccessPath(session, `/audits/${auditId}/artifacts/findings`)) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  if (auditId === "demo") {
    return jsonDownload(findingsToRows(runDemoAudit().findings), `traceready-findings-${auditId}.json`);
  }
