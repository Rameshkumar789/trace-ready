...
export async function GET(_request: Request, { params }: { params: Promise<{ auditId: string }> }) {
  ...
  if (!session || !canAccessPath(session, `/audits/${auditId}/artifacts/workbook`)) {
  ...
  if (auditId === "demo") {
    ...
    body.set(workbook);
    return artifactDownload(body, `traceready-audit-${auditId}.xlsx`, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  }
