...
from traceready_ingestion.api.config import ServiceSettings, load_settings
from traceready_ingestion.api.errors import register_error_handlers
from traceready_ingestion.api.observability import add_request_logging, build_logger
from traceready_ingestion.api.readiness import build_readiness_report
from traceready_ingestion.api.security import require_internal_token
...
class AuditJobSliceRequest(BaseModel):
    worker_id: str = Field(default="vercel-cron")
...
def create_app(settings: ServiceSettings | None = None) -> FastAPI:
    ...
    api = FastAPI(
        title="TraceReady Python Backend",
    ...
    if loaded_settings.allowed_origins:
        api.add_middleware(
            ...
            allow_headers=["authorization", "content-type", "x-request-id", "x-traceready-internal-token"],
        )
